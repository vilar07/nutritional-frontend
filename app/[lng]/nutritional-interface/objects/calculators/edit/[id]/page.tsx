...
export default function editCalculator({ params }: PropsType) {
    ...
    useEffect(() => {
        ...
        .then(response => {
            ...
            setVariableToCalculate(response.data.variable_to_calculate);
    ...
    async function editCalculator() {
        if (!title || !subtitle || !description || !images || !equation || !variableToCalculate || !time_of_day_relevance || !season_relevance) {
        ...
        try {
            axios.put(`http://localhost:4000/objects/calculator/${params.id}`, data)
            .then(response => {
                console.log(response);
                axios.put(`http://localhost:4000/objects/associate/calculator/${title}`, associations)
                .then(response => {
                    console.log(response);
    ...
    async function deleteCalculator() {
        try {
            const response = await axios.delete(`http://localhost:4000/objects/calculator/${params.id}`);
            console.log(response);
            window.location.href = '/nutritional-interface/objects/calculators';
        } catch (error) {
            console.log(error);
            ...
            />
                <div className="w-full grid grid-cols-1 gap-2 p-4">
                    <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-2">
                        <div className="w-full grid grid-cols-1 gap-2 border-2 rounded-xl bg-white">
                            <div className="w-full text-center p-4">
                                <h1 className="mt-2 mb-2 font-bold text-xl text-green2">Edit Calculator</h1>
                                <hr className="mb-4" />
                                ...
                                <div className="w-full grid grid-cols-1 gap-1 p-4">
                                    ...
                                    {variableToCalculate &&
                                        <div className="w-full grid grid-cols-5 mt-4 bg-gray-200 p-2 border-2 rounded-xl">
                                            <div className="w-full col-span-1 flex items-center justify-center">
                                                <span className="text-sm">{variableToCalculate}</span>
                                            </div>
                                            <div className="w-full col-span-1 flex items-center justify-center">
                                                <span className="text-xl">=</span>
                                            </div>
                                            <div className="w-full col-span-3 whitespace-wrap">
                                                <div className="w-full p-float-label">
                                ...
                                <div>
                                    ...
                                    <div className="w-full p-4 grid grid-cols-1 ">
                                        <div className="w-full p-4">
                                            <div className="w-full ">
                                                <h1 className="text-xl font-bold text-green2 ">Choose Characteristis to be associated with the calculator</h1>
                                            </div>
                                        </div>
                                        <div className="w-full text-black text-opacity-60 p-float-label">
                                            <Dropdown 
                                    ...
                                    <div className="w-full p-4 grid grid-cols-1 gap-2">
                                        <h1 className="text-xl font-bold text-green2 " >Table with the characteristics to be collected that are associated with the Calculator</h1>
                                        <ul>
                        ...
                        <div className="w-full grid grid-cols-1 gap-2 border-2 rounded-xl bg-white whitespace-wrap">
                            <div className="w-full text-center p-4 ">
                                ...
                                {description && 
                                <div className="w-full grid grid-cols-1 gap-4 px-8 mt-8">
                                    <div className="w-full flex items-center justify-center text-left  p-4 border-2 rounded-xl">
                    ...
                    <div className="w-full text-left flex flex-row">
                        <Button onClick={editCalculator} label="Edit Calculator" className='p-3 bg-gray-500 text-white hover:bg-white hover:text-gray-500 ' text raised/> 
                        <Button onClick={deleteCalculator} label="Delete Calculator" className='mx-2 p-3 bg-red-500 text-white hover:bg-white hover:text-red-500 ' text raised/>
                    </div>
                </div>
            </div>
        </div>
