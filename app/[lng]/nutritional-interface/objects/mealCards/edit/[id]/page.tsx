...
export default function editMealCard({ params }: PropsType) {
    useEffect(() => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:4000/objects/mealCard/${params.id}`)
        .then(response => {
            console.log("Meal Card:", response.data);

        // Create a new object by copying over properties from response.data
        const modifiedRecipe = {
            ...response.data,
            total_ingredients: response.data.number_ingridients, // Rename the field
            name: response.data.title,
            id: response.data.ID,
            // You can add more modifications here if needed
        };

        // Delete the original property if necessary
        delete modifiedRecipe.number_ingridients;
        delete modifiedRecipe.title;
        delete modifiedRecipe.ID;

        console.log("Modified Recipe:", modifiedRecipe);
        // Set the modified recipe to recipeSelected
        setRecipeSelected(modifiedRecipe);
    ...
    async function editMealCard() {
        ...
        try {
            const response = await axios.put(`http://localhost:4000/objects/mealCard/${params.id}`, data);
            console.log(response);
            
            const response2 = await axios.put(`http://localhost:4000/objects/associate/mealCard/${recipeSelected.name}`, associations);
    ...
    async function deleteMealCard() {
        try {
            const response = await axios.delete(`http://localhost:4000/objects/mealCard/${params.id}`);
            console.log(response);
            window.location.href = '/nutritional-interface/objects/mealCards';
        } catch (error) {
            console.log(error);
                ...
                <div className="w-full grid grid-cols-1 gap-2 px-4">
                    ...
                    {recipeSelected === null ? (
                        <div className="w-full grid grid-cols-4 gap-1 mt-4">
                            ...
                            <div className="w-full col-span-3 grid grid-cols-3 gap-2 overflow-y-scroll">
                        ...
                        <div className="w-full grid grid-cols-1 gap-2">
                            ...
                            <div className="w-full flex-col justify-start items-center">
                                ...
                                {recipeSelected && (
                                    <div  className="w-full grid grid-cols-1 gap-2 border-2 rounded-xl p-2">
                                        <div className="w-full flex items-center justify-center">
                            ...
                            <div className="w-full">
                                <div className="w-full p-4">
                                    <div className="w-full lg:w-1/3 text-black text-opacity-60 p-float-label">
                                        <Dropdown 
                                ...
                                {possibleOptions && possibleOptions.length > 0 && (
                                    <>
                                        <div className="w-full p-4">
                                            <div className="w-1/3 text-black text-opacity-60 p-float-label">
                                            <Dropdown 
                                    ...
                                    <ul>
                                        {selectedCharacteristics.map((item: any, index: any) => (
                                            <div key={index} className="w-full grid grid-cols-3 gap-2 border-2">
                                                <div className="w-full border-r-2 p-2 text-black font-bold">{item.characteristic}</div>
                        ...
                        <Button onClick={editMealCard} label="Edit MealCard" className='p-3 bg-gray-500 text-white hover:bg-white hover:text-gray-500 ' text raised/> 
                        <Button onClick={deleteMealCard} label="Delete MealCard" className='mx-2 p-3 bg-red-500 text-white hover:bg-white hover:text-red-500 ' text raised/>
                    </div>
                        </div>
                    )}
                </div>
