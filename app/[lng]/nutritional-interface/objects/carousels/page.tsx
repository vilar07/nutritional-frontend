...
export default function Carousels({ params: { lng } }: PropsType) {
    ...
    useEffect(() => {
        let apiUrl = 'http://localhost:4000/objects/carousels';
            ...
            <div className="w-full px-8 items-center justify-center flex lg:hidden">
                ...
                <div className="w-full grid grid-cols-1 gap-2 mt-8 p-8">
                    <div className="w-full justify-start items-center">
                        <a href="/nutritional-interface/objects/carousels/create"><Button label="Create a Carousel" className='p-3 bg-gray-500 text-white hover:bg-white hover:text-gray-500 ' text raised/></a>
                    </div>
                    <div className="w-full flex flex-col justify-start items-start mt-4">
                        <h1 className="text-xl text-black text-opacity-70">Filter Carousels by</h1>
                    ...
                    <div className="w-full h-[500px] overflow-y-scroll">
                        ...
                        {carousels.length>0 ? ( 
                            carousels.map((carousel) => (
                                <div key={carousel.ID} className="bg-green2 flex flex-col justify-between h-full p-4 border-2 rounded-xl text-center bg-green-200 hover:scale-105 shadow-xl">
                                    <h1 className="text-xl text-black text-opacity-70 font-bold">{carousel.title}</h1>
                                    <div className="w-full p-3">
                                        <Slider {...settings}>
                                            {carousel.items.map((item: any) => (
                                                <div key={item.ID}>
                                                    <div className="bg-white border-4 border-gray-500 rounded-xl my-4 ">
                                                        <a href={`/carousel/${carousel.ID}`}>
                                                            <Image
                                    ...
                                    <hr className="mt-2"/>
                                    <a href={`/nutritional-interface/objects/carousels/edit/${carousel.ID}`}>
                                    ...
                                    <div className="w-full flex justify-end items-center mt-2">
                                        <p className="text-black text-opacity-45">{carousel.views} Views</p>
                                    </div>
                                </div>
                            ))
                        ) : (
