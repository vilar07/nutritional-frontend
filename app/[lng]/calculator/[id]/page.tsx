...
export default function calculator({ params }: PropsType) {
        ...
        axios.get(`http://localhost:4000/objects/calculators`) 
            .then((response) => {
                ...
                if (urlPathname.includes('/calculator')) {
                    ...
                    console.log("filtered Calculatours without current:", filteredCalculatorWithoutCurrent)
                } else {
    ...
    const [result, setResult] = useState<string | null>(null); // State for storing the result

    //Incrementing the views of the calculator
    ...
    useEffect(() => {
        if (!isMounted.current) {
            ...
            const fetchData = async () => {
                try {
                    axios.get(`http://localhost:4000/objects/calculator/${params.id}`)
                        .then(response => {
                            ...
                            axios.put(`http://localhost:4000/objects/views/calculator/${params.id}`)
                                .then(response => {
    ...
    // Check if calculator.description is defined before calling formatDescriptionHTML
    if (calculator.description) {
        // Call formatDescriptionHTML and assign the result to the formattedDescriptionHTML variable
        formattedDescriptionHTML = formatDescriptionHTML(calculator.description);
    }
    ...
    console.log("Variable Values:", variableValues); // Log the state before the loop
    console.log("Variables:", variables);

    // Check if all inputs are filled
    for (const variable of variables) { // Change variableValues to variables
        ...
        //---------------------------------//
        useEffect(() => {
            axios.get(`http://localhost:4000/objects/ratings/calculator/${params.id}`)
            .then(response => {
        ...
        useEffect(() => {
            if(email){
                axios.get(`http://localhost:4000/objects/ratings/calculator/${params.id}/${email}`)
                .then(response => {
        ...
        async function sendRating(){
            ...
            axios.post(`http://localhost:4000/objects/ratings/calculator/${params.id}/${email}`, {rating: ratingValue} )
            .then(response => {
        ...
        async function updateRating(){
            axios.put(`http://localhost:4000/objects/ratings/calculator/${params.id}/${email}`, {rating: editRating})
            .then(response => {
        ...
        async function deleteRating(){
            axios.delete(`http://localhost:4000/objects/ratings/calculator/${params.id}/${email}`)
            .then(response => {
    ...
    return (
        <>
                ...
                <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div className="w-full flex items-center justify-center  text-center lg:text-left flex-col">
                        <div className="w-full mb-4">
                            <h1 className="w-full text-6xl text-balance text-green2">{calculator.title}</h1>
                        </div>
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-8 px-4 lg:px-32  mt-24">
                <div className="w-full flex items-center justify-center text-left shadow-xl mb-6  p-6 border-2 rounded-xl hover:scale-105">
                    <div className="w-full" dangerouslySetInnerHTML={{ __html: formattedDescriptionHTML }} />
                </div>
                <div className="w-full flex flex-col items-center justify-center text-center bg-gray-200 p-4 border-2 rounded-xl hover:scale-105">
                    <div className="w-full items-center justify-center text-3xl text-center mb-8">Calculate {calculator.variable_to_calculate}</div>
                    {variables.map(variable => (
                        <div key={variable} className="mb-4 w-full flex items-center justify-center">
                    ...
                    {result !== "Error" && result ? (
                        <div className="w-full grid grid-cols-1 gap-4 px-16 mt-4">
                            <div className="w-full flex flex-col items-center justify-center text-left bg-gray-100 p-4 border-2 rounded-xl hover:scale-105">
                                ...
                                <div className="w-full text-2xl text-green2">{result} {calculator.variable_to_calculate}</div>
                            </div>
                        </div>
                    ) : result ? (
                        <div className="w-full grid grid-cols-1 gap-4 px-16 mt-4">
            ...
            <div className="w-full flex flex-col xl:flex-row justify-start px-4 lg:px-32 lg:mt-4">
            ...
            <div className="w-full grid grid-cols-1 gap-1 mt-8 justify-start px-4 lg:px-32 ">
                <div className="w-full flex justify-center lg:justify-start text-center items-center">
                    <h1 className="w-full flex text-4xl font-bold text-center text-green2 drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]">Recommended for You !</h1>
                </div>
            </div>
            <div className="w-full justify-start px-4 lg:px-32 grid grid-cols-1 mt-4">
            ...
            <div className="w-full justify-center px-4 lg:px-32 grid grid-cols-1 mt-4">
                <div className="w-full flex flex-col items-center justify-center">
                    {recipes.length > 0 ? (
                        <>
                        <h1 className="w-full flex items-center justify-center text-center flex-row text-3xl font-bold text-green2 drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]">Our Recommended Recipes for You! <FaRegLaughWink size={'1.2em'} color={"green"} className='ml-2' /> </h1>
                        {diseases.length > 0 || allergies.length > 0 || diets.length > 0 ? (
                        <div className="w-full flex flex-col justify-center items-center text-center">
                            <div className="w-full flex flex-row">
                                <h3 className="w-full text-black text-opacity-30">Recommended for people:<Button onClick={() => setVisible1(true)} label="Check" className='ml-2 px-2 bg-gray-500 text-white hover:bg-white hover:text-gray-500 rounded-full ' text raised /></h3>
